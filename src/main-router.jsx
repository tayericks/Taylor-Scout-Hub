...
function ToolGate({tool}) {
  ...
  useEffect(() => {
    let cancelled = false;
    async function load() {
      if (!configured) { if (!cancelled) setState({ loading: false, show: null, error: 'Taylor Scout Supabase is not configured.' }); return; }
...
function installCrewMapsLinkGuard() {
  document.addEventListener('click', event => {
    ...
    let href; try { href = new URL(anchor.href, window.location.origin); } catch { return; }
    ...
    ['show','showId','showName','fromHub'].forEach(key => { const value = href.searchParams.get(key); if (value) target.searchParams.set(key, value); });
    target.searchParams.set('fromHub', '1'); window.location.assign(target.toString());
  }, true);
}
