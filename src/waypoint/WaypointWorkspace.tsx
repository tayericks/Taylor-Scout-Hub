...
export default function WaypointWorkspace({show,onBack}:Props){
  ...
  useEffect(()=>{let dead=false;(async()=>{setLoading(true);setError('');
    ...
    const list=(docs||[]).map((row:any)=>{const c=row.payload?.context||{};return attachCoords({id:c.id||row.tool_key.replace('waypoint:',''),toolKey:row.tool_key,...c})});
  ...
  async function createFromCalendar(){if(!selectedEvent)return;const id=crypto.randomUUID();const l=locationFor({locationId:selectedEvent.locationId,calendarEventId:selectedEvent.id,location:selectedEvent.location,address:selectedEvent.address});const lat=Number(l?.metadata?.latitude),lng=Number(l?.metadata?.longitude);const context:SchematicContext={id,toolKey:`waypoint:${id}`,calendarEventId:selectedEvent.id,locationId:selectedEvent.locationId||l?.id||'',episode:selectedEvent.episode||'',scenes:selectedEvent.scenes||'',unit:selectedEvent.unit||'',set:selectedEvent.set||'',location:selectedEvent.location||'Untitled Location',address:selectedEvent.address||l?.address||'',shootStart:selectedEvent.shootStart||'',shootEnd:selectedEvent.shootEnd||selectedEvent.shootStart||'',latitude:Number.isFinite(lat)?lat:undefined,longitude:Number.isFinite(lng)?lng:undefined};
  ...
  async function deleteSchematic(){const target=schematics.find(s=>s.id===existingId);if(!target)return;if(!window.confirm(`Delete ${target.location||'this Waypoint'}${target.set?` — ${target.set}`:''}? This cannot be undone.`))return;const {error:e}=await supabase.from('tool_documents').delete().eq('show_id',show.id).eq('tool_key',target.toolKey);if(e){setError(e.message);return}setSchematics(v=>v.filter(s=>s.id!==target.id));setExistingId('')}
  const dashboard=()=>{if(onBack)return onBack();window.location.assign(link(HUB_URL,show))};
  if(active)return <WaypointApp show={show} onBack={()=>setActive(null)} onDashboard={dashboard} toolKey={active.toolKey} context={enrich(active)}/>;
